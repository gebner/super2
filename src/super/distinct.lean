...
private meta def dedup_core : clause → list literal → clause
| c@⟨clause_type.ff, _⟩ _ := c
| c@⟨clause_type.imp a b, prf⟩ ctx :=
  match ctx.index_of' (literal.neg a) with
  | some i := dedup_core ⟨b, prf (expr.var i)⟩ ctx
  | none :=
    let ⟨b', prf'⟩ :=
      dedup_core ⟨b, prf.lift_vars 0 1 (expr.var 0)⟩ (literal.neg a :: ctx) in
...
| ⟨clause_type.or clause_type.ff b, prf⟩ ctx :=
  dedup_core ⟨b, `((@false_or %%b.to_expr).mp %%prf)⟩ ctx
| ⟨clause_type.or a clause_type.ff, prf⟩ ctx :=
  dedup_core ⟨a, `((@or_false %%a.to_expr).mp %%prf)⟩ ctx
| c@⟨clause_type.or a b, prf⟩ ctx :=
  if (a.literals ∩ b.literals) = [] then
    let ⟨a', prfa'⟩ := dedup_core ⟨a, expr.var 0⟩ (literal.neg a.to_expr :: ctx) in
    let ⟨b', prfb'⟩ := dedup_core ⟨b, expr.var 0⟩ (literal.neg b.to_expr :: ctx) in
    ...
    `(@or_imp_congr %%a.to_expr %%a'.to_expr %%b.to_expr %%b'.to_expr
      ...
      %%prf)⟩ in
    if res.is_or_ff_clean then res else dedup_core res ctx
  else
    match a with
    | clause_type.or a1 a2 :=
      dedup_core ⟨clause_type.or a1 (clause_type.or a2 b),
        `((@or.assoc %%a1.to_expr %%a2.to_expr %%b.to_expr).mp %%prf)⟩ ctx
    | clause_type.atom a :=
      let ⟨b', prf'⟩ := dedup_core ⟨b, expr.var 0⟩
      ...
      let res : clause := ⟨clause_type.or (clause_type.atom a) b',
        `(@or_imp_congr_right_strong %%a %%b.to_expr %%b'.to_expr
          ...
          %%prf)⟩ in
      if res.is_or_ff_clean then res else dedup_core res ctx
    | clause_type.ff := undefined_core "dedup_core ff"
    | clause_type.imp a1 clause_type.ff :=
      dedup_core ⟨clause_type.imp a1 b, `((@imp_iff_or_not %%a1 %%b.to_expr).mpr)⟩ ctx
    | clause_type.imp a1 a2 :=
      dedup_core ⟨clause_type.or (clause_type.or (clause_type.imp a1 clause_type.ff) a2) b,
        `(@or_imp_congr_left %%b.to_expr %%a.to_expr (¬ %%a1 ∨ %%a2.to_expr)
          %%(`(iff.mp $ @imp_iff_or_not %%a1 %%a2.to_expr)) %%prf)⟩ ctx
    end

meta def dedup (c : clause) : clause :=
if ¬ c.is_or_ff_clean ∨ c.has_dups then dedup_core c [] else c
